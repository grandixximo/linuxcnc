...
**-c**:: Enable checksum output which is necessary for debugging the checksum generator function.
 Do not rely on this feature since it will be removed once the generator is implemented.

**-n**:: Force being silent and not printing any output except of errors.
...
It is safer to copy and paste from https://raw.githubusercontent.com/LinuxCNC/linuxcnc/devel/src/hal/user_comps/xhc-whb04b-6/example-configuration.md[].
