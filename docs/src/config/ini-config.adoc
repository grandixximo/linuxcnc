...
* `MAX_SPINDLE_0_OVERRIDE = 1.0` - (real)The maximum spindle override the user may select.
...
* `G64_DEFAULT_ANGULAR_TOLERANCE =` _n_ (Default: 0)
  Default E value for G64 if E is not called out, in degrees.
...
* `MACHINE_MOVES_NEED_MACHINE_FRAME = 0` (bool, Default: 0) +
  When set, 'G53', 'G28', 'G30', 'G28.1' and 'G30.1' are refused whenever the kinematics in force is not the machine frame type: the type the module flags as such, the identity kinematics on a machine whose slides line up with its frame, and neither the tool centre point nor a raw joint identity on one whose slides do not.
  These codes work in the world of the kinematics in force, which on a table-side kinematics turns with the table; the machine frame positions are 'G53.5', 'G28.5' and 'G30.5'.
  See <<gcode:g53,G53>>.
...
* `PLANNER_TYPE = 0` - (((PLANNER TYPE))) Selects the trajectory planner type: 0 = trapezoidal (default), 1 = S-curve with jerk limiting.
  S-curve planning is only active when `PLANNER_TYPE = 1` AND `MAX_LINEAR_JERK` is 1.0
  or above; a smaller value falls back to trapezoidal, with an operator message.
...
* `SPEED_HUMP_TIME = 0.0` - (((SPEED HUMP TIME))) Between two slow points close together, such as the corners of a run of short moves, the S-curve planner speeds up as far as it can and slows down again at once.
  Such a speed hump saves little time and shakes the machine like a vibration.
  A speed hump shorter than this many seconds is left out: the motion holds its speed instead, for a single speed change.
  0 (the default) leaves every hump in; 0.02 to 0.05 is a typical value.
  The <<gcode:g64,G64 R word>> divides it: R0.5 leaves out humps up to twice as long.
  A speed-up just before a stop is kept.
  Only the `tpnextmod` trajectory planner honors it.
* `SINGULAR_MODE = STOP` - (((SINGULAR MODE))) What the `tpnextmod` trajectory planner does with a move that passes a singular pose of the kinematics, where a joint could follow the path only at less than `SINGULAR_FLOOR` of the programmed feed.
  `STOP` (the default) refuses the move when it is queued, with an error naming its line and the joint, and the motion stops before it.
  `FLOOR` runs the move at `SINGULAR_FLOOR` of its feed there, faster than that joint's limits allow, with a message naming its line and the joint.
* `SINGULAR_FLOOR = 0.001` - (((SINGULAR FLOOR))) The fraction of the programmed feed below which a move counts as passing a singular pose, see `SINGULAR_MODE`.
...
* `TYPE = LINEAR` - (enum) The type of this axis, either `LINEAR` or `ANGULAR`.
  ...
  The `tpnextmod` trajectory planner applies the `G64 P` blend tolerance to the linear axes and `G64 E` to the angular ones.
...
+
`MAX_VELOCITY` and `MAX_ACCELERATION` bound the letter in the world of the kinematics in force.
The joints are bound on their own: every move is capped at what its joints can follow within their `[JOINT_N]` limits, read through the kinematics module's Jacobian along the move, and a world jog the same way every servo cycle, so on a kinematics that is not the identity a move or a jog runs at the pace of its slowest joint whatever these say.
See the <<cha:switchable-kinematics,Switchable Kinematics>> chapter, INI file limit settings.
...
+
`MIN_LIMIT` and `MAX_LIMIT` together are a box in the machine frame: the carriage, or the flange of a robot, in machine coordinates, the frame that does not move when a head tilts or a tool is loaded.
On a kinematics module that declares its machine frame type (see the <<cha:switchable-kinematics,Switchable Kinematics>> chapter) every move is checked against the box where its joints put the machine frame, whatever kinematics type is in force: a tool centre point move whose tip lies past the box with the carriage inside it is accepted, and a 'G53.5' to a carriage position past the box is refused.
The travel itself is the `[JOINT_N]` limits; the box is a restriction the configuration adds inside it.
A module that declares no machine frame type keeps the box on the world of the kinematics in force.
...
[[sub:ini:sec:mtconnect]]
=== [MTCONNECT] Section(((INI File,Sections,[MTCONNECT] Section)))

Optional.  Configures the `mtconnect-agent`(1) userspace component, which
publishes machine status, kinematics and tool data using the MTConnect standard.
Load it from a HAL file with `loadusr -W mtconnect-agent`.  The MTConnect device
model is generated automatically from the `[TRAJ]`, `[KINS]`, `[AXIS_<letter>]`
and `[JOINT_<num>]` sections, so only the keys below are required.

* `ENABLE = 1` - (bool) Enable the agent.  Default 1.
* `DEVICE_NAME = my_mill` - (string) MTConnect device name.
  Default: `[EMC]MACHINE`.
* `UUID = 0e2c...` - (string) MTConnect device uuid.
* `TRANSPORT = http` - (string) Comma-separated list of transports:
  `http`, `mqtt`, `shdr`.  Default `http`.
* `HTTP_PORT = 5000` - (int) Port for the embedded HTTP agent.
* `HTTP_BIND = 127.0.0.1` - (string) Interface to bind the HTTP agent to.
  Default 127.0.0.1 (loopback only); set to `0.0.0.0` to expose on the network.
* `SAMPLE_HZ = 10` - (real) Poll and publish rate in Hz.
* `MQTT_BROKER`, `MQTT_PORT`, `MQTT_PREFIX`, `MQTT_USERNAME`, `MQTT_PASSWORD` -
  Settings for the standard MTConnect MQTT binding.  Requires the
  `python3-paho-mqtt` package.

The embedded HTTP agent serves `/probe`, `/current`, `/sample` and `/assets`, and
referenced geometry under `/models/`.  The spindle speed
band from `[SPINDLE_0]MIN_FORWARD_VELOCITY`/`MAX_FORWARD_VELOCITY` is advertised
as an MTConnect Specification.  See `mtconnect-agent`(1) for the full reference.
