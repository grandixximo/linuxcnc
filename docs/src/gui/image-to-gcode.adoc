...
to make AXIS automatically invoke image-to-gcode when you open a
...
The standard 'sim/axis.ini' configuration file is already prepared
this way.
...
create the G-code. Depending on the image size and options chosen, this
may take from a few seconds to a few minutes. If you are loading the
image in AXIS, the G-code will automatically be loaded and previewed
...
Image-to-gcode can optionally perform roughing passes. The depth of
